...
//Jesse C. - Originally undefined. Lets see if this fixes 2-point touch
...
//Jesse C. - This was before that define - "defined(ABS_MT_TRACKING_ID)  && "
//I change this hear to make sure CY_USR_TRACKING_ID is set for true MT instead of 2 point
//since if it is 0 then cyttsp-i2c.c flips.
#if defined(CY_USE_MT_TRACK_ID)
	#define CY_USE_TRACKING_ID	1
#else
	//was 0. just MAKE SURE IT'S ONE IF YOU WANT MOAR POINTS
	#define CY_USE_TRACKING_ID	1
/* define only if not defined already by system;
 * value based on linux kernel 2.6.30.10
 */
//Maybe I should just define this myself instead of the hack above? Meh.
...
// Jesse C: This was originally 2. Should now track up to 4 touches instead of stock 2
#define CY_NUM_ST_TCH_ID		4

/* maximum number of concurrent MT track IDs */
// Jesse C: 4 originally. Hell, why not
#define CY_NUM_MT_TCH_ID		8
...
/* TrueTouch Standard Product Gen3 (Txx3xx) test data interface definition */

struct cyttsp_gen3_testdata_t {
	u8 hst_mode;
	u8 tt_mode;
	u8 tt_stat;
	u16 x1 __attribute__ ((packed));
