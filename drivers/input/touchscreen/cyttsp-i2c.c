...
struct cyttsp {
	...
	//Jesse C. originally defined as CY_NUM_MT_TCH_ID
	u16 prv_mt_tch[4];
	//Jesse C. - Is this 2 with the touch issue? Lets make it 4
	u16 prv_mt_pos[CY_NUM_TRK_ID][4];
...
void cyttsp_xy_worker(struct work_struct *work)
{
	...
	//Jesse C. - Pretty positive this 2 has something to do with it
	u16 cur_mt_pos[CY_NUM_TRK_ID][4];
	/* if NOT CY_USE_TRACKING_ID then
	 * only uses CY_NUM_MT_TCH_ID positions */
